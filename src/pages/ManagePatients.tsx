...
export default function ManagePatients() {
  ...
  const fetchPatients = async () => {
    ...
    // ----- [AUTO-SYNC] -----
    try {
      const { data: orphans, error: syncError } = await supabase
        .from('appointments')
      ...
      if (orphans && orphans.length > 0) {
        ...
        for (const [whatsapp, name] of patientMap) {
          const { data: ptData, error: upsertError } = await supabase
            .from('patients')
            .upsert({
              doctor_id: user.id,
              full_name: name,
          ...
          if (upsertError) {
             console.error(`Falha ao sincronizar paciente ${name}:`, upsertError.message);
             continue;
          }
      ...
      console.error('Erro Crítico no Auto-Sync:', err.message);
    }
    // ----- [/AUTO-SYNC] -----
